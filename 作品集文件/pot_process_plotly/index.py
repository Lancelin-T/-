...
from common.process_plot import plotly_gongyirpt_multi_subplots
from common.mysql_handler import get_gongyirpt_range_data  # 使用K2数据源
...
# #     # 本地图片路径
#     image_path = f'./fig/铝一3分厂/工艺曲线/默认图片.png'
#
#     # 使用K2DataFrameDB获取数据（函数内部会获取 start_date-30天 到 end_date 的数据）
#     data_gongyprt = get_gongyirpt_range_data_from_k2(selected_pot, start_date, end_date)
#
...
if query_button or (selected_pot and start_date and end_date):
    ...
    date_gongyprt = get_gongyirpt_range_data(selected_pot, start_date, end_date)

    start_datetime = pd.to_datetime(start_date).date()
    end_datetime = pd.to_datetime(end_date).date()

    date_gongyprt = date_gongyprt[
        (date_gongyprt['k_ts'] >= start_datetime) &
        (date_gongyprt['k_ts'] <= end_datetime)
    ].copy()

    logger.info(f"过滤后的数据行数：{len(date_gongyprt)}")

    fig = plotly_gongyirpt_multi_subplots(date_gongyprt)
